# -*- coding: utf-8 -*-
"""
STEP 4 — County review figures for one-by-one manual checking.
...
Run:  python src/04_county_review.py            # resume (skip existing)
      python src/04_county_review.py --overwrite # full rebuild
"""
import os, sys
...
#
# COMPUTED ON THE FULL PANEL, INCLUDING NON-AGRICULTURAL COUNTIES, on purpose:
# 03_clean_panel.py imputes against C.provincial_median_ln(agg, v) over the whole
# frame, so this is the line the imputation actually rode.  Recomputing it on the
# agricultural subset would draw a reference the algorithm never used, and a
# county could look like it diverges from "its province" when it does not.
PROVMED = {}
for v in C.IO_VARS:
    lnv = np.log(pd.to_numeric(m[v], errors="coerce").where(lambda s: s > 0))
    PROVMED[v] = lnv.groupby([m["SID"], m["year"]]).median()

# ...but only the AGRICULTURAL counties are drawn, since those are the analysis
# sample.  --all-counties renders every unit in the cleaned panel instead.
if "--all-counties" not in sys.argv:
    if "ag_county" not in m.columns:
        sys.exit("cleaned panel lacks `ag_county` — re-run 02 and 03")
    n_all = m.countyid.nunique()
    m = m[m["ag_county"] == 1].copy()
    print(f"rendering the AGRICULTURAL sample: {m.countyid.nunique():,d} of "
          f"{n_all:,d} counties  (--all-counties for the rest)")
else:
    print(f"rendering ALL {m.countyid.nunique():,d} counties in the cleaned panel")
