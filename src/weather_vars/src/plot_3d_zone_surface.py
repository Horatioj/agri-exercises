...
3D production surface per agricultural zone.
  x = log10 total input
  y = rzsm_gs (root-zone soil moisture, growing season)
...
OUT  = Path('/Users/zhangxinzhen/Desktop/TFP/Code/output/3d_zone_surface')
...
df = df.loc[mask, inputs + ['GVP_allagr_impute', 'rzsm_gs', 'zone37_main', 'year']].copy()
...
def draw(ax, sub, title, *, label=True, show_pts=True, cmap=cm.viridis):
    Xg, Yg, Zg, r2, beta = fit_and_grid(sub)

    # bounds (push the walls farther out so the projected curves sit clearly
    # *behind* the surface, then draw them BEFORE the surface so the surface
    # naturally overlays them)
    ...
    def quad_fit(a, b):
        """OLS b = c0 + c1*a + c2*a^2."""
        A_ = np.column_stack([np.ones_like(a), a, a ** 2])
        c, *_ = np.linalg.lstsq(A_, b, rcond=None)
        return c

    xx = sub['log_input'].values
    yy = sub['rzsm_gs'].values
    zz = sub['log_output'].values

    # Three input-output slices of the surface at p10, p50, p90 of climate.
    # If the three lines are parallel -> climate has NO moderation effect on the
    # input-output relationship.  If they fan out (slopes differ) -> climate
    # moderates the input elasticity:
    #   - high-climate line steeper than low-climate -> climate amplifies input
    #   - shallower -> climate dampens input
    slice_specs = [
        (np.percentile(yy, 10), '#fdcc8a', 'low rzsm (p10)'),
        (np.percentile(yy, 50), '#fc8d59', 'mid rzsm (p50)'),
        (np.percentile(yy, 90), '#b30000', 'high rzsm (p90)'),
    ]
    ...
    # Left wall (yz at x=x_lo): Output ~ rzsm, fit on projected (y, z) data
    c = quad_fit(yy, zz)
    z_y = c[0] + c[1] * yg + c[2] * yg ** 2
    ax.plot(np.full_like(yg, x_lo), yg, z_y, color='#1f77b4', lw=2.0, zorder=0)

    # Bottom (xy at z=z_lo): Input ~ rzsm   (causal direction: rzsm -> input)
    # Treat rzsm as the regressor so the curve gives a single input for each
    # soil-moisture level (consistent with the assumption that input is
    # endogenous to rzsm, not the other way round).
    c = quad_fit(yy, xx)
