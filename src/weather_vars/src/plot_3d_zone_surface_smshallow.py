...
3D production surface per agricultural zone.
  x = log10 total input
  y = sm_shallow_gs (shallow soil moisture, growing season)
...
OUT  = Path('/Users/zhangxinzhen/Desktop/TFP/Code/output/3d_zone_surface_smshallow')
...
mask = (df[inputs] > 0).all(axis=1) & (df['GVP_allagr_impute'] > 0) \
       & df['sm_shallow_gs'].notna() & df['zone37_main'].notna() & df['valid_climate_sample']
df = df.loc[mask, inputs + ['GVP_allagr_impute', 'sm_shallow_gs', 'zone37_main', 'year']].copy()
...
def draw(ax, sub, title, *, label=True, show_pts=True, cmap=cm.viridis):
    ...
    yy = sub['sm_shallow_gs'].values
    zz = sub['log_output'].values

    # Three input-output slices of the surface at p10, p50, p90 of sm_shallow.
    slice_specs = [
        (np.percentile(yy, 10), '#fdcc8a', 'low sm_shallow (p10)'),
        (np.percentile(yy, 50), '#fc8d59', 'mid sm_shallow (p50)'),
        (np.percentile(yy, 90), '#b30000', 'high sm_shallow (p90)'),
    ]
    ...
    # Left wall (yz at x=x_lo): Output ~ sm_shallow, fit on projected (y, z) data
    c = quad_fit(yy, zz)
    z_y = c[0] + c[1] * yg + c[2] * yg ** 2
    ax.plot(np.full_like(yg, x_lo), yg, z_y, color='#1f77b4', lw=2.0, zorder=0)

    # Bottom (xy at z=z_lo): Input ~ sm_shallow   (causal direction: sm_shallow -> input)
    ...
    # endogenous to sm_shallow, not the other way round).
    c = quad_fit(yy, xx)
    ...
    if show_pts:
        ...
        ax.scatter(s['log_input'], s['sm_shallow_gs'], s['log_output'],
                   s=3, color='black', alpha=0.18, depthshade=False)

    ax.set_title(title, fontsize=9, pad=2)
    if label:
        ax.set_xlabel('Input',  fontsize=7, labelpad=-4)
        ax.set_ylabel('sm_shallow',   fontsize=7, labelpad=-4)
