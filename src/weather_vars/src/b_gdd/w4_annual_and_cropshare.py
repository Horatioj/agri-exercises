...
def main():
    ...
    if not a.skip_annual:
        ...
        for i, y in enumerate(years):
            for name, mm in (("gdd_annual", MG[i]), ("hdd_annual", MH[i])):
                tot = np.where(np.isnan(mm).any(axis=0), np.nan, np.nansum(mm, axis=0))
                b = tot.astype("float32").copy(); b[np.isnan(b)] = -9999.0
                with rasterio.open(tmp, "w", **prof) as d:
                    d.write(b, 1)
                wm = exact_extract(str(tmp), g, ops=["weighted_mean"], weights=str(cw_f),
        ...
        tmp.unlink(missing_ok=True)
        gd = pd.concat([r_ for r_ in rows if "gdd_annual" in r_.columns], ignore_index=True)
        hd = pd.concat([r_ for r_ in rows if "hdd_annual" in r_.columns], ignore_index=True)
        ann = gd.merge(hd, on=["code", "year"]).rename(columns={"code": "map_code"})
