...
def main():
    ...
    io = io[io["ag_county"] == 1]          # weather is joined to the ag sample
    ...
    # SOIL MOISTURE.  This script owns io_weather_panel.csv, so the soil-moisture
    # table has to be joined HERE.  It previously was not, while 40_cps_prep.py,
    # 52_* and 54_* all read `sm_0_28` off this panel -- so the CPS decomposition
    # was silently reading a column that the current pipeline never wrote.
    sm_f = GDD_DIR / "county_year_soilmoisture.csv"
    if sm_f.exists():
        sm = pd.read_csv(sm_f)
        dup = [c for c in sm.columns if c in out.columns and c not in ("map_code", "year")]
        sm = sm.drop(columns=dup)
        out = out.merge(sm, on=["map_code", "year"], how="left")
        got = [c for c in sm.columns if c not in ("map_code", "year")]
        n_s = int(out["sm_0_28"].notna().sum()) if "sm_0_28" in out.columns else 0
        print(f"    soil moisture joined: {len(got)} columns, "
              f"{n_s:,d} rows ({100*n_s/max(len(out),1):.1f}%)")
    else:
        print(f"    WARNING: {sm_f.name} not found -- panel has NO soil moisture; "
              f"run 13_county_soil_moisture.py first")
