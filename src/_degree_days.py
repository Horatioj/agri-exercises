...
plus a check against the daily-MEAN method the original B step used.

WHY THE METHOD MATTERS
----------------------
The original B step computed one number per day from the daily MEAN:
...
USAGE
-----
    from _degree_days import dd_above, gdd_hdd
...
Run this file directly to validate against real temperature rasters:
    python _degree_days.py --zip "Z:/weather data/temperature" --year 1982
"""
...
def gdd_daily_mean(tmin, tmax, base: float = BASE_T, cap: float = CAP_T):
    """The superseded daily-mean method, for comparison: piecewise on the mean."""
